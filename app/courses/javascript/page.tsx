<<<<<<< HEAD
// app/javascript/page.tsx
'use client';

import { javascriptSheet } from "./data/javascriptSheet";
import PracticePanel from "./components/PracticePanel";
import QuestScrollSection from "./components/ScrollSection";
import StoryPanel from "./components/StoryPanel";            // Separate: storyline only
import XPTracker from "../components/XPTracker";               // Separate: XP + stats only
...
export default function JavascriptCoursePage() {
  return (
    <div className="space-y-10">
      ...
      {/* XP stats only */}
      <XPTracker totalXP={0} />
      {/* Scrolls as Quest cards */}
      <div className="space-y-8">
        {javascriptSheet.map((scroll) => (
          <section key={scroll.id} className="mb-8">
            <div className="flex items-center gap-3 mb-4">
              <span className="text-3xl">{scroll.icon}</span>
              <h2 className="text-2xl font-bold text-white drop-shadow">
                {scroll.title}
              </h2>
              <span className="bg-purple-800/60 px-2 py-0.5 rounded text-purple-200 font-semibold text-xs ml-2">
                {scroll.estimatedTime} · {scroll.quests.length} Quests
              </span>
            </div>
            <p className="text-purple-200 mb-4">{scroll.description}</p>
            <QuestScrollSection quests={scroll.quests} />
            <div className="mt-4">
              <PracticePanel quests={scroll.quests} />
            </div>
          </section>
        ))}
      </div>
